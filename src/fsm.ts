...
export type Options = { delay?: number };

type Send = (event: string, options?: Options, ctx?: object) => void;
type Action = (send: Send, ctx?: object) => void;

export type Guard = (ctx: object) => boolean;
export type Transition = { target: string; guard?: Guard };
export type State = {
  on?: { [key: string]: string | Transition };
  entry?: Action;
};
...
export function fsm(initial: string, config: Record<string, State>): Machine {
  ...
  function transition(event: string, ctx?: object) {
  ...
  // The action creator
  function send(event: string, options?: Options, ctx?: object): void {
